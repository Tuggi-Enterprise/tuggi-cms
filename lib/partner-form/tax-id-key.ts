 ...
 * database.
 *
 * THE OWNER IS `core.partner_form_submissions.tax_id_normalized`, a `GENERATED ALWAYS ... STORED`
 * column written in migration `20260814140000_issue341_proposta_do_parceiro_em_formulario_unico`:
 *
 ...
 * It is a MIRROR, not a second owner: when the two disagree the column wins, and the migration's
 * verify is where they are compared against the same vectors.
 *
 ...
 * the mutation `tests/api/partner-form.test.ts` exists to catch.
 *
 * WHY IT IS NOT `[^0-9]`: the CNPJ is alphanumeric since 2026-07-31 (IN RFB nº 2.229/2024) —
