...
import type { GeoPoint } from '../types/interfaces';
import { calculateDistance, calculateBearing } from './calculations';
...
export function findClosestPointOnStreet(streetCoordinates: number[][], targetLat: number, targetLng: number): {
  ...
  let closestIndex = 0;
  let minDistance = calculateDistance({ lat: targetLat, lng: targetLng }, { lat: streetCoordinates[0][1], lng: streetCoordinates[0][0] });
  
  for (let i = 1; i < streetCoordinates.length; i++) {
    const distance = calculateDistance({ lat: targetLat, lng: targetLng }, { lat: streetCoordinates[i][1], lng: streetCoordinates[i][0] });
...
export function calculateStreetConfidence(tags: any, distance: number): number {
  ...
  if (tags.highway) {
    ...
    confidence += (highwayBonus as any)[tags.highway] || 0.4;
  }
...
export function calculateStreetDirection(coordinates: number[][], pointIndex: number, isReverse: boolean = false): number {
  ...
  return calculateBearing({ lat: fromPoint[1], lng: fromPoint[0] }, { lat: toPoint[1], lng: toPoint[0] });
}
...
export function findClosestPointIndexOnStreet(coordinates: number[][], poiLat: number, poiLng: number): number {
  let closestIndex = 0;
  let minDistance = calculateDistance({ lat: poiLat, lng: poiLng }, { lat: coordinates[0][1], lng: coordinates[0][0] });
  
  for (let i = 1; i < coordinates.length; i++) {
    const distance = calculateDistance({ lat: poiLat, lng: poiLng }, { lat: coordinates[i][1], lng: coordinates[i][0] });
    if (distance < minDistance) {
      minDistance = distance;
      closestIndex = i;
    }
