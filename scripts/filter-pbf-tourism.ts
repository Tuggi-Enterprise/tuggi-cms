...
async function main() {
  ...
  const skipEtapa3 = args.includes("--skip-etapa3") ||
    args.includes("--fase-2-only");
  ...
  if (fase5Only) {
    ...
    if (!etapa4File) {
      console.error(
        "❌ Erro: --fase-5-only requer um arquivo GeoJSON como argumento",
      );
      console.error(
        "   Exemplo: deno run filter-pbf-tourism.ts arquivo.geojson --fase-5-only",
      );
      Deno.exit(1);
    }
    ...
    const etapa4_5FinalPath = await executeEtapa4_5_Unified(
      processor,
      etapa4File,
      outputDir,
      timestamp,
    );
    const etapa5_6FinalPath = await executeEtapa5_6(
      etapa4_5FinalPath,
      outputDir,
      timestamp,
    );
    ...
    console.log("");
    console.log(
      "✅ Processo concluído - ETAPA 4-5.7 UNIFICADA e ETAPA 5.6 executadas!",
    );
    console.log(
      "   💡 A ETAPA 4-5.7 unifica 8 fases em uma única passagem para melhor performance",
    );
    return;
  }
  ...
  if (!skipEtapa1) {
    ...
    console.log(
      "   1. Incluir categorias de interesse (tourism, historic, natural, etc.)",
    );
    console.log(
      "   2. Incluir igrejas católicas (amenity=place_of_worship + denomination católica)",
    );
    console.log("   3. Excluir highways (mesmo com tourism/historic)");
    console.log("");
    console.log(
      "💡 Estratégia: 3 operações paralelas + merge + remoção de highways",
    );
    console.log("");
    ...
    console.log(
      `   Tags: ${
        ETAPA1_TAGS.slice(0, 5).join(", ")
      }... (${ETAPA1_TAGS.length} total)`,
    );
    ...
    try {
      ...
      const categoriasPath = await processor.extractTags(
        inputFile,
        ETAPA1_TAGS,
        false,
      );
      // Re-filter para garantir precisão (também sem omit-referenced)
      etapa1Categorias = await processor.extractTags(
        categoriasPath,
        ETAPA1_TAGS,
        false,
      );
    ...
    console.log(
      "   Critério: amenity=place_of_worship + denomination=catholic/roman_catholic",
    );
    ...
    try {
      ...
      const placeOfWorshipPath = await processor.extractTags(inputFile, [
      ...
      for (const denom of catholicDenominations) {
        try {
          const catholicPath = await processor.extractTags(placeOfWorshipPath, [
            denom,
          ], false);
      ...
      if (catholicFiles.length > 0) {
        etapa1Igrejas = join(
          outputDir,
          `temp-catholic-merged-${timestamp}.osm.pbf`,
        );
      ...
      console.log(
        `   ⚠️  Erro ao filtrar igrejas católicas: ${
          error instanceof Error ? error.message : String(error)
        }`,
      );
    ...
    const etapa1Merged = join(
      outputDir,
      `temp-etapa1-merged-${timestamp}.osm.pbf`,
    );
    ...
    console.log(
      "📋 ETAPA 1.4: Removendo highways (mesmo com tourism/historic)...",
    );
    console.log(
      "   Regra: Highways são infraestrutura, não POIs para turistas",
    );
  ...
  console.log("🎯 Lógica:");
  console.log(
    "   - MANTER: tourism OU historic (mesmo com restrições de acesso)",
  );
  console.log(
    "   - EXCLUIR: SEM tourism/historic E access=no/residential/private OU residential=yes",
  );
  console.log("");
  console.log("📋 Estratégia: Filtrar diretamente do arquivo da ETAPA 1");
  console.log(
    "   Usando --invert-match para remover objetos com access restritivo",
  );
  ...
  try {
    // IMPORTANTE: Não usar --omit-referenced para preservar geometria de ways/relations
    etapa2aPath = await processor.extractTags(
      etapa2BasePath,
      tourismHistoricTags,
      false,
    );
  ...
  console.log(
    "📋 ETAPA 2.2: Filtrando objetos SEM tourism/historic E SEM access restritivo...",
  );
  ...
  try {
    ...
    // IMPORTANTE: Não usar --omit-referenced para preservar geometria de ways/relations
    const semAccessRestritivo = await processor.extractTags(
      etapa2BasePath,
      restrictiveAccessTags,
      false,
      true,
    ); // true = invertMatch
    ...
    // IMPORTANTE: Não usar --omit-referenced para preservar geometria de ways/relations
    etapa2bPath = await processor.extractTags(
      semAccessRestritivo,
      otherCategoryTags,
      false,
    );
  ...
  console.log(
    "   (Combinando tourism/historic + outras categorias sem restrições)",
  );
  console.log("");
  ...
  console.log(
    "📋 ETAPA 2.4: Re-filtrando após merge para garantir precisão...",
  );
  ...
  const etapa2ReFilteredPath = join(
    outputDir,
    `etapa2-refiltered-${timestamp}.osm.pbf`,
  );
  ...
  try {
    ...
    const reFilteredPath = await processor.extractTags(
      etapa2MergePath,
      tagsToKeep,
      false,
    );
  ...
  console.log(
    "📋 ETAPA 2.5: Removendo objetos com access restritivo sem tourism/historic...",
  );
  console.log(
    "   (Remove objetos que têm access restritivo E não têm tourism/historic)",
  );
  console.log("");
  ...
  const etapa2Merge2Path = join(
    outputDir,
    `etapa2-merge2-${timestamp}.osm.pbf`,
  );

  try {
    // 1. Extrair tourism/historic (mantém todos, mesmo privados)
    // IMPORTANTE: Não usar --omit-referenced para preservar geometria
    const tourismHistoricPath = await processor.extractTags(
      etapa2ReFilteredPath,
      tourismHistoricTags,
      false,
    );
    ...
    // 2. Extrair objetos SEM access restritivo (mantém todos sem restrições)
    // IMPORTANTE: Não usar --omit-referenced para preservar geometria
    const semAccessRestritivoPath = await processor.extractTags(
      etapa2ReFilteredPath,
      restrictiveAccessTags,
      false,
      true,
    ...
    // 3. Merge dos dois conjuntos
    await processor.mergeFiles(
      [tourismHistoricPath, semAccessRestritivoPath],
      etapa2Merge2Path,
    );
  ...
  // O merge pode incluir objetos relacionados que não foram explicitamente filtrados
  console.log(
    "📋 ETAPA 2.6: Re-filtrando após merge para garantir precisão...",
  );
  console.log("   (Remove objetos relacionados que não têm as tags corretas)");
  console.log("");

  const etapa2FinalPath = join(
    outputDir,
    `etapa2-access-filtered-${timestamp}.osm.pbf`,
  );
  ...
  console.log(
    "   Critérios: tourism/historic OU wikipedia/wikidata OU description/website OU name",
  );
  ...
  console.log(
    "📋 ETAPA 3.2: Filtrando natural com indicadores de importância...",
  );
  console.log("");
  console.log(
    "⚠️  Nota: natural=peak com ele >= 500m será tratado no banco de dados",
  );
  ...
  try {
    ...
    const tempNaturalPath = await processor.extractTags(
      etapa2FinalPath,
      naturalTags,
      false,
    );
    ...
    const naturalWithNamePath = await processor.extractTags(
      tempNaturalPath,
      naturalWithNameTags,
      false,
    );
    const naturalNamePath = await processor.extractTags(naturalWithNamePath, [
      "name",
    ], false);
    filesToMerge.push(naturalNamePath);

    // Filtrar natural com wikipedia
    const naturalWikipediaPath = await processor.extractTags(tempNaturalPath, [
      "wikipedia",
    ], false);
    filesToMerge.push(naturalWikipediaPath);

    // Filtrar natural com wikidata
    const naturalWikidataPath = await processor.extractTags(tempNaturalPath, [
      "wikidata",
    ], false);
    filesToMerge.push(naturalWikidataPath);

    // Filtrar natural com description
    const naturalDescriptionPath = await processor.extractTags(
      tempNaturalPath,
      ["description"],
      false,
    );
    ...
    const naturalWebsitePath = await processor.extractTags(tempNaturalPath, [
      "website",
    ], false);
    filesToMerge.push(naturalWebsitePath);

    console.log(
      `✅ ETAPA 3.2 complete: ${filesToMerge.length - 1} arquivos criados`,
    );
    console.log("");
  } catch (error) {
    console.error(
  ...
  console.log(
    "📋 ETAPA 3.3: Filtrando leisure com indicadores de importância...",
  );
  console.log("");
  ...
  try {
    ...
    // IMPORTANTE: Não usar --omit-referenced para preservar geometria de áreas (ways/relations)
    const tempLeisurePath = await processor.extractTags(
      etapa2FinalPath,
      leisureTags,
      false,
    );
    ...
    const leisureNamePath = await processor.extractTags(tempLeisurePath, [
      "name",
    ], false);
    filesToMerge.push(leisureNamePath);

    // Filtrar leisure com wikipedia
    const leisureWikipediaPath = await processor.extractTags(tempLeisurePath, [
      "wikipedia",
    ], false);
    filesToMerge.push(leisureWikipediaPath);

    // Filtrar leisure com wikidata
    const leisureWikidataPath = await processor.extractTags(tempLeisurePath, [
      "wikidata",
    ], false);
    filesToMerge.push(leisureWikidataPath);

    // Filtrar leisure com description
    const leisureDescriptionPath = await processor.extractTags(
      tempLeisurePath,
      ["description"],
      false,
    );
    ...
    const leisureWebsitePath = await processor.extractTags(tempLeisurePath, [
      "website",
    ], false);
    filesToMerge.push(leisureWebsitePath);

    // Filtrar leisure=park com park:type
    const leisureParkPath = await processor.extractTags(tempLeisurePath, [
      "leisure=park",
    ], false);
    const leisureParkTypePath = await processor.extractTags(leisureParkPath, [
      "park:type",
    ], false);
    filesToMerge.push(leisureParkTypePath);

    // Filtrar leisure=park com operator
    const leisureOperatorPath = await processor.extractTags(leisureParkPath, [
      "operator",
    ], false);
    filesToMerge.push(leisureOperatorPath);

    console.log(
      `✅ ETAPA 3.3 complete: ${filesToMerge.length - 6} arquivos criados`,
    );
    console.log("");
  } catch (error) {
    console.error(
  ...
  console.log(
    "📋 ETAPA 3.4: Filtrando aeroway com indicadores de importância...",
  );
  console.log("");

  try {
    // Primeiro, filtrar todos os aeroway
    const tempAerowayPath = await processor.extractTags(etapa2FinalPath, [
      "aeroway=aerodrome",
    ], true);

    // Filtrar aeroway com name
    const aerowayNamePath = await processor.extractTags(tempAerowayPath, [
      "name",
    ], true);
    filesToMerge.push(aerowayNamePath);

    // Filtrar aeroway com wikipedia
    const aerowayWikipediaPath = await processor.extractTags(tempAerowayPath, [
      "wikipedia",
    ], true);
    filesToMerge.push(aerowayWikipediaPath);

    // Filtrar aeroway com wikidata
    const aerowayWikidataPath = await processor.extractTags(tempAerowayPath, [
      "wikidata",
    ], true);
    filesToMerge.push(aerowayWikidataPath);

    // Filtrar aeroway com iata
    const aerowayIataPath = await processor.extractTags(tempAerowayPath, [
      "iata",
    ], true);
    filesToMerge.push(aerowayIataPath);

    // Filtrar aeroway com icao
    const aerowayIcaoPath = await processor.extractTags(tempAerowayPath, [
      "icao",
    ], true);
    filesToMerge.push(aerowayIcaoPath);

    console.log(
      `✅ ETAPA 3.4 complete: ${filesToMerge.length - 11} arquivos criados`,
    );
    console.log("");
  } catch (error) {
    console.error(
  ...
  console.log(
    "📋 ETAPA 3.5: Filtrando amenity=theatre com indicadores de importância...",
  );
  console.log("");

  try {
    // Primeiro, filtrar todos os amenity=theatre
    const tempTheatrePath = await processor.extractTags(etapa2FinalPath, [
      "amenity=theatre",
    ], true);

    // Filtrar theatre com historic
    const theatreHistoricPath = await processor.extractTags(tempTheatrePath, [
      "historic",
    ], true);
    filesToMerge.push(theatreHistoricPath);

    // Filtrar theatre com wikipedia
    const theatreWikipediaPath = await processor.extractTags(tempTheatrePath, [
      "wikipedia",
    ], true);
    filesToMerge.push(theatreWikipediaPath);

    // Filtrar theatre com wikidata
    const theatreWikidataPath = await processor.extractTags(tempTheatrePath, [
      "wikidata",
    ], true);
    filesToMerge.push(theatreWikidataPath);

    // Filtrar theatre com name
    const theatreNamePath = await processor.extractTags(tempTheatrePath, [
      "name",
    ], true);
    filesToMerge.push(theatreNamePath);

    // Filtrar theatre com description
    const theatreDescriptionPath = await processor.extractTags(
      tempTheatrePath,
      ["description"],
      true,
    );
    ...
    const theatreWebsitePath = await processor.extractTags(tempTheatrePath, [
      "website",
    ], true);
    filesToMerge.push(theatreWebsitePath);

    console.log(
      `✅ ETAPA 3.5 complete: ${filesToMerge.length - 16} arquivos criados`,
    );
    console.log("");
  } catch (error) {
    console.error(
  ...
  const etapa3FinalPath = join(
    outputDir,
    `etapa3-importance-filtered-${timestamp}.osm.pbf`,
  );
  ...
  if (fase4Only) {
    ...
    if (!etapa3File) {
      console.error(
        "❌ Erro: --fase-4-only requer um arquivo PBF como argumento",
      );
      console.error(
        "   Exemplo: deno run filter-pbf-tourism.ts arquivo.pbf --fase-4-only",
      );
      Deno.exit(1);
    }
    // Usar função antiga para compatibilidade
    etapa4_5FinalPath = await executeEtapa4(
      processor,
      etapa3File,
      outputDir,
      timestamp,
    ...
    etapa4_5FinalPath = await executeEtapa4_5_Unified(
      processor,
      etapa3FinalPath,
      outputDir,
      timestamp,
  ...
  if (etapa4_5FinalPath) {
    etapa5_6FinalPath = await executeEtapa5_6(
      etapa4_5FinalPath,
      outputDir,
      timestamp,
    );
  ...
  console.log(
    "   • Adição de igrejas católicas (amenity=place_of_worship + denomination católica)",
  );
  console.log("");
}
...
async function executeEtapa4_5_Unified(
  ...
  console.log("=".repeat(60));
  console.log(
    "📋 ETAPA 4-5.7 UNIFICADA: Filtros de Valor Turístico (Refined Elite Filter)",
  );
  console.log("=".repeat(60));
  console.log(
    "🎯 Objetivo: Remover POIs sem valor turístico real (Filtro 'Elite')",
  );
  console.log("");
  console.log("📊 Lógica Unificada:");
  console.log(
    "   • Blacklist Estrita de Nomes (Anti-Comercial/Anti-Burocracia)",
  );
  console.log(
    "   • Exigência de Fama (Wiki/Histórico) para Hotéis e Atrações Genéricas",
  );
  console.log(
    "   • Filtro de Categorias (Parques, Igrejas, Mercados) com regras Contextuais",
  );
  console.log("");
  ...
  function shouldFilterPOI(poi: any): { remove: boolean; reason?: string } {
    ...
    for (const term of FILTER_CONFIG.GLOBAL_BLOCKLIST) {
      if (nameLower.includes(term)) {
        // Exception: "Center" / "Centro" allowed for Famous/Cultural spots
        if (term === "center" || term === "centro") {
          const isCultural = props.tourism === "museum" || props.tourism === "attraction";
          const isGenericInfo = props.tourism === "information";
          if (isFamous || (isCultural && !isGenericInfo)) continue; 
        }

        // Exception: "Horta" allowed if Famous/Attraction
        if (term === "horta" && (isFamous || props.tourism === "attraction")) continue;

        // Special rule for Shopping: Always remove unless it has Wikipedia fame
    ...
    if (props.leisure) {
      ...
      if (isPark) {
        ...
        const isArea = poi.geometry && (poi.geometry.type === "Polygon" || poi.geometry.type === "MultiPolygon");
        if (!isArea && !isFamous && !props.tourism) {
    ...
    // --- 13. INFRASTRUCTURE (Towers) ---
    if (["tower", "water_tower"].includes(props.man_made)) {
      if (!isFamous && !props.tourism && !props.historic && !hasReference) {
        return { remove: true, reason: "Category: Torre/Infraestrutura sem valor turístico" };
      }
    }
    ...
    if (words.length === 1) {
      ...
      return { remove: true, reason: "Strict: Nome de palavra única sem referência global" };
    }
  ...
  const tempGeoJsonSeqPath = join(
    outputDir,
    `temp-etapa4_5_unified-${timestamp}.geojsonseq`,
  );
  ...
  console.log(
    "📋 ETAPA 4-5.7.2: Filtrando POIs (aplicando Critérios de Elite)...",
  );
  console.log("");

  const etapa4_5FinalPath = join(
    outputDir,
    `etapa4_5-unified-filtered-${timestamp}.geojson`,
  );
  ...
  try {
        ...
        while ((newlineIndex = buffer.indexOf("\n")) !== -1) {
          ...
          if (line.length > 0) {
            try {
              ...
              const check = shouldFilterPOI(feature);
              ...
              if (lineCount % 1000 === 0) {
                console.log(
                  `   Processados: ${lineCount.toLocaleString()} features...`,
                );
              }
            } catch (e) {
    ...
    console.log("");
    console.log(
      `   Total de POIs antes do filtro: ${totalFeatures.toLocaleString()}`,
    );
    console.log(`   POIs removidos: ${removedFeatures.toLocaleString()}`);
    console.log(
      `   POIs válidos restantes: ${validFeatures.length.toLocaleString()}`,
    );
    console.log(
      `   Redução: ${Math.round((removedFeatures / totalFeatures) * 100)}%`,
    );
    console.log("");
    ...
    if (Object.keys(removalStats).length > 0) {
      ...
      sortedStats.forEach(([reason, count]) => {
        ...
        console.log(
          `   • ${reason}: ${count.toLocaleString()} (${percentage}%)`,
        );
      });
      console.log("");
    }
...
async function executeEtapa4(
  ...
  console.log("=".repeat(60));
  console.log(
    "🎯 Objetivo: Remover POIs problemáticos identificados na análise",
  );
  ...
  const tempGeoJsonSeqPath = join(
    outputDir,
    `temp-etapa4-${timestamp}.geojsonseq`,
  );
  ...
  try {
    ...
    const command = new Deno.Command("osmium", {
      args: [
        ...
        inputPath,
        "-f",
        "geojsonseq", // GeoJSONSeq = NDJSON (Newline Delimited JSON)
        "-o",
        tempGeoJsonSeqPath,
        "--overwrite",
      ],
  ...
  const filteredGeoJsonPath = join(
    outputDir,
    `etapa4-filtered-${timestamp}.geojson`,
  );
  ...
  try {
    ...
    function shouldKeepPOI(poi: any): boolean {
      ...
      const isCatholicChurch = props["amenity"] === "place_of_worship" && (
        ...
        (props.religion &&
          (props.religion.toLowerCase() === "catholic" ||
            props.religion.toLowerCase() === "roman_catholic"))
      );
      ...
      if (
        !hasName && !hasReference && !isTourismOrHistoric && !isCatholicChurch
      ) {
        return false;
      }
    ...
    console.log(
      "   Usando osmium para filtrar diretamente no PBF antes da análise...",
    );
    console.log("");
    ...
    console.log(
      "   Processando GeoJSONSeq linha por linha (streaming nativo)...",
    );
    console.log("");
    ...
    console.log(
      `   POIs removidos: ${
        (totalFeatures - validFeatures.length).toLocaleString()
      }`,
    );
    console.log(
      `   POIs válidos restantes: ${validFeatures.length.toLocaleString()}`,
    ...
    console.log(
      `   Redução: ${
        Math.round((totalFeatures - validFeatures.length) / totalFeatures * 100)
      }%`,
    );
    console.log("");
  ...
  const etapa4FinalPath = join(
    outputDir,
    `etapa4-business-filtered-${timestamp}.osm.pbf`,
  );
  ...
  try {
    ...
    const finalGeoJsonPath = join(
      outputDir,
      `etapa4-final-${timestamp}.geojson`,
    );
    ...
    console.log(
      `   Nota: Este GeoJSON pode ser usado diretamente para importação no banco`,
    );
    console.log(
      `   Se precisar de PBF, pode converter manualmente ou usar o GeoJSON diretamente`,
    );
    console.log("");
...
async function executeEtapa5(
  ...
  console.log("=".repeat(60));
  console.log(
    "🎯 Objetivo: Remover POIs genéricos sem valor para audioguia turístico",
  );
  console.log("");
  ...
  function shouldRemovePOI(poi: any): boolean {
    ...
    if (
      genericPatterns.genericNames.some((p) => p.test(name)) && !hasReference
    ) {
      const isReallyGeneric = name.length <= 10 && !hasDescription &&
        !hasWebsite;
      if (isReallyGeneric) {
        return true; // Remover
      }
    ...
    if (
      genericPatterns.cemeteries.some((p) => p.test(name)) ||
      props.amenity === "grave_yard" || props.amenity === "cemetery" ||
      props.landuse === "cemetery"
    ) {
      const isFamous = famousNames.cemeteries.some((f) =>
    ...
    if (
      ...
      genericPatterns.stadiums.some((p) => p.test(name))
    ) {
    ...
    if (
      ...
      const isHistoric = nameLower.includes("antiga") ||
        nameLower.includes("antigo");
      if (!isHistoric) {
        return true; // Remover
      }
    }
    ...
    if (props.aeroway === "aerodrome") {
      ...
      if (
        !props.iata && !props.icao && !hasReference &&
        (!hasName || name.length < 5)
      ) {
        return true; // Remover
      }
    }
  ...
  try {
    ...
    if (inputPath.endsWith(".geojson")) {
      ...
      tempGeoJsonSeqPath = join(
        outputDir,
        `temp-etapa5-${timestamp}.geojsonseq`,
      );
      const file = await Deno.open(tempGeoJsonSeqPath, {
        write: true,
        create: true,
        truncate: true,
      });
      ...
      file.close();
      console.log(
        `✅ GeoJSON convertido para GeoJSONSeq: ${tempGeoJsonSeqPath}`,
      );
  ...
  const etapa5FinalPath = join(
    outputDir,
    `etapa5-rigorous-filtered-${timestamp}.geojson`,
  );
...
async function executeEtapa5_1(
  ...
  function isBankTouristicValid(
    name: string,
    amenity: string,
    tourism: string,
  ): boolean {
    // Verificar se é uma exceção válida
    const isException = bancosTuristicosValidos.some((pattern) =>
      pattern.test(name)
    );
  ...
  try {
    if (inputPath.endsWith(".geojson")) {
      ...
      tempGeoJsonSeqPath = join(
        outputDir,
        `temp-etapa5_1-${timestamp}.geojsonseq`,
      );
      const file = await Deno.open(tempGeoJsonSeqPath, {
        write: true,
        create: true,
  ...
  console.log(
    "📋 ETAPA 5.1.2: Filtrando POIs de bancos sem valor turístico...",
  );
  console.log("");

  const etapa5_1FinalPath = join(
    outputDir,
    `etapa5_1-banks-filtered-${timestamp}.geojson`,
  );
...
async function executeEtapa5_2(
  ...
  console.log("=".repeat(60));
  console.log(
    "🎯 Objetivo: Remover vias de trâfego (não são POIs para audioguia)",
  );
  console.log("");
  console.log("📊 Critérios de remoção:");
  console.log(
    "   • Nomes que começam com: Estrada, Rua, Avenida, Rodovia, Via, etc.",
  );
  console.log(
    "   • Nomes que contêm padrões de vias (ex: Av., R., Est., Rod.)",
  );
  ...
  function isRoad(name: string, props: any): boolean {
    ...
    const startsWithVia = padroesInicioVia.some((pattern) =>
      pattern.test(name)
    );
    if (startsWithVia) {
  ...
  try {
    if (inputPath.endsWith(".geojson")) {
      ...
      tempGeoJsonSeqPath = join(
        outputDir,
        `temp-etapa5_2-${timestamp}.geojsonseq`,
      );
      const file = await Deno.open(tempGeoJsonSeqPath, {
        write: true,
        create: true,
  ...
  const etapa5_2FinalPath = join(
    outputDir,
    `etapa5_2-roads-filtered-${timestamp}.geojson`,
  );
  ...
  try {
    ...
    console.log(
      `   POIs removidos (vias): ${removedFeatures.toLocaleString()}`,
    );
    console.log(
      `   POIs válidos restantes: ${validFeatures.length.toLocaleString()}`,
    );
...
async function executeEtapa5_3(
  ...
  console.log("=".repeat(60));
  console.log(
    "🎯 Objetivo: Remover POIs genéricos que não adicionam valor ao audioguia",
  );
  console.log("");
  console.log("📊 Critérios de remoção:");
  console.log(
    "   • Nomes apenas genéricos: 'Mirante', 'Monumento', 'Busto', etc.",
  );
  console.log(
    "   • Nomes genéricos sem contexto: 'Mirante 1', 'Estátua', 'Cruzeiro'",
  );
  console.log(
    "   • Mantém POIs com contexto: 'Mirante de Buenos Aires', 'Monumento ao Almirante Negro'",
  );
  console.log("");
  ...
  function hasGenericContext(
    ...
    if (name.length < minLength) {
      ...
      if (
        /^\s*(mirante|monumento|busto|estátua|estatua|escultura|memorial|cruzeiro)\s*(\d+|$)/i
          .test(name)
      ) {
      ...
      if (
        nameLower === termo || nameLower === termo + " 1" ||
        nameLower === termo + " 2" || nameLower === termo + " 3"
      ) {
      ...
      if (
        nameLower.includes(" de ") || nameLower.includes(" do ") ||
        nameLower.includes(" da ") ||
        nameLower.includes(" das ") || nameLower.includes(" dos ") ||
        nameLower.includes(" ao ") ||
        nameLower.includes(" em ") || nameLower.includes(" na ") ||
  ...
  function shouldRemoveGenericPOI(poi: any): boolean {
    ...
    if (
      props.wikipedia || props.wikidata || props.historic === "monument" ||
    ...
    const nomeMuitoGenerico =
      /^(mirante|monumento|busto|estátua|estatua|escultura|memorial|cruzeiro|marco)(\s+\d+)?$/i;
  ...
  try {
    if (inputPath.endsWith(".geojson")) {
      ...
      tempGeoJsonSeqPath = join(
        outputDir,
        `temp-etapa5_3-${timestamp}.geojsonseq`,
      );
      const file = await Deno.open(tempGeoJsonSeqPath, {
        write: true,
        create: true,
  ...
  const etapa5_3FinalPath = join(
    outputDir,
    `etapa5_3-generic-filtered-${timestamp}.geojson`,
  );
  ...
  try {
    ...
    console.log(
      `   POIs removidos (genéricos sem contexto): ${removedFeatures.toLocaleString()}`,
    );
    console.log(
      `   POIs válidos restantes: ${validFeatures.length.toLocaleString()}`,
    );
...
async function executeEtapa5_4(
  ...
  console.log("=".repeat(60));
  console.log(
    "🎯 Objetivo: Remover POIs específicos que não adicionam valor ao audioguia",
  );
  console.log("");
  console.log("📊 Critérios de remoção:");
  ...
  try {
    if (inputPath.endsWith(".geojson")) {
      ...
      tempGeoJsonSeqPath = join(
        outputDir,
        `temp-etapa5_4-${timestamp}.geojsonseq`,
      );
      const file = await Deno.open(tempGeoJsonSeqPath, {
        write: true,
        create: true,
  ...
  console.log(
    "📋 ETAPA 5.4.2: Filtrando POIs específicos sem valor turístico...",
  );
  console.log("");

  const etapa5_4FinalPath = join(
    outputDir,
    `etapa5_4-specific-filtered-${timestamp}.geojson`,
  );
  ...
  try {
    ...
    console.log(
      `   POIs removidos (específicos sem valor turístico): ${removedFeatures.toLocaleString()}`,
    );
    console.log(
      `   POIs válidos restantes: ${validFeatures.length.toLocaleString()}`,
    );
...
async function executeEtapa5_5(
  ...
  console.log("=".repeat(60));
  console.log(
    "🎯 Objetivo: Remover POIs de infraestrutura e serviços que não são turísticos",
  );
  console.log("");
  console.log("📊 Critérios de remoção:");
  ...
  function shouldRemoveInfrastructurePOI(poi: any): boolean {
    ...
    if (
      props.aeroway === "aerodrome" || props.aeroway === "airport" ||
    ...
    if (
      name.includes("aeródromo") || name.includes("aerodromo") ||
    ...
    // Verificar por nome também
    if (
      (name.includes("escola") || name.includes("colégio") ||
    ...
    // 3. Serviços Públicos
    if (
      props.amenity === "police" || props.amenity === "fire_station" ||
    ...
    if (
      props.amenity === "hospital" || props.amenity === "clinic" ||
    ...
    // 7. Infraestrutura
    if (
      props.amenity === "parking" || props.amenity === "charging_station" ||
    ...
    if (
      props.leisure === "track" || props.leisure === "pitch" ||
    ...
    if (
      props.landuse === "commercial" || props.landuse === "industrial" ||
  ...
  try {
    if (inputPath.endsWith(".geojson")) {
      ...
      tempGeoJsonSeqPath = join(
        outputDir,
        `temp-etapa5_5-${timestamp}.geojsonseq`,
      );
      const file = await Deno.open(tempGeoJsonSeqPath, {
        write: true,
        create: true,
  ...
  const etapa5_5FinalPath = join(
    outputDir,
    `etapa5_5-infrastructure-filtered-${timestamp}.geojson`,
  );
  ...
  try {
    ...
    console.log(
      `   POIs removidos (infraestrutura/serviços): ${removedFeatures.toLocaleString()}`,
    );
    console.log(
      `   POIs válidos restantes: ${validFeatures.length.toLocaleString()}`,
    );
...
async function executeEtapa5_6(
  ...
  console.log(
    "   • POIs com mesmo nome E mesma localização (< 500m) = duplicata real",
  );
  console.log(
    "   • POIs com mesmo nome mas localizações diferentes (> 500m) = POIs diferentes (manter todos)",
  );
  console.log(
    "   • Para duplicatas reais, manter apenas 1 entrada (com mais informações)",
  );
  console.log("");
  ...
  try {
    if (inputPath.endsWith(".geojson")) {
      ...
      tempGeoJsonSeqPath = join(
        outputDir,
        `temp-etapa5_6-${timestamp}.geojsonseq`,
      );
      const file = await Deno.open(tempGeoJsonSeqPath, {
        write: true,
        create: true,
  ...
  const etapa5_6FinalPath = join(
    outputDir,
    `etapa5_6-deduped-${timestamp}.geojson`,
  );
  ...
  try {
    ...
    console.log(
      `   Total de POIs carregados: ${allFeatures.length.toLocaleString()}`,
    );
    console.log("");
    ...
    porNome.forEach((pois, name) => {
      ...
      pois.filter((p) => getCoordenadas(p) === null).forEach((p) =>
        validFeatures.push(p)
      );
    });

    console.log(
      `   Total de POIs antes da deduplicação: ${allFeatures.length.toLocaleString()}`,
    );
    console.log(
      `   Duplicatas removidas: ${duplicatesRemoved.toLocaleString()}`,
    );
    console.log(
      `   POIs válidos restantes: ${validFeatures.length.toLocaleString()}`,
    );
    console.log(
      `   Redução: ${
        Math.round((duplicatesRemoved / allFeatures.length) * 100)
      }%`,
    );
    console.log("");
...
async function executeEtapa5_7(
  ...
  console.log("=".repeat(60));
  console.log(
    "🎯 Objetivo: Remover POIs genéricos sem valor turístico identificável",
  );
  console.log("");
  console.log("📊 Critérios:");
  console.log(
    "   • REMOVER: POIs com nome de 1 palavra SEM referências externas E SEM descrição",
  );
  console.log(
    "   • MANTER: POIs com Wikipedia/Wikidata (têm valor identificável)",
  );
  ...
  try {
    if (inputPath.endsWith(".geojson")) {
      ...
      tempGeoJsonSeqPath = join(
        outputDir,
        `temp-etapa5_7-${timestamp}.geojsonseq`,
      );
      const file = await Deno.open(tempGeoJsonSeqPath, {
        write: true,
        create: true,
  ...
  const etapa5_7FinalPath = join(
    outputDir,
    `etapa5_7-single-word-filtered-${timestamp}.geojson`,
  );
  ...
  try {
    ...
    console.log(
      `   POIs mantidos: ${keptCount.toLocaleString()} (${
        ((keptCount / allFeatures.length) * 100).toFixed(2)
      }%)`,
    );
    console.log(
      `   POIs removidos: ${removedCount.toLocaleString()} (${
        ((removedCount / allFeatures.length) * 100).toFixed(2)
      }%)`,
    );
    console.log("");
    ...
    if (removedPOIs.length > 0) {
      ...
      removedPOIs.slice(0, 10).forEach((poi, index) => {
        const props = poi.properties || {};
        console.log(
          `   ${index + 1}. "${props.name || "sem nome"}" - ${
            props.tourism || props.historic || props.leisure || props.natural ||
            "sem categoria"
          }`,
        );
      });
    ...
    if (keptWithValueExamples.length > 0) {
      ...
      keptWithValueExamples.slice(0, 5).forEach((poi, index) => {
        ...
        console.log(
          `   ${index + 1}. "${props.name || "sem nome"}" - Mantido por: ${
            reasons.join(", ")
          }`,
        );
      });
