/**
 * create-orlando-disney-free.ts
 *
 * Cria a rota "Disney Sem Ingressos — A Magia Gratuita de Orlando" no banco de dados.
 *
 * O script:
 *   1. Define 7 landmarks dos hotéis temáticos e áreas públicas da Disney
 ...
 *
 * Uso:
 *   npx tsx scripts/create-orlando-disney-free.ts
 *   npx tsx scripts/create-orlando-disney-free.ts --force   # recria se já existir
 */
...
const ROUTE_NAME        = 'Disney Sem Ingressos — Um Dia Inteiro na Magia Gratuita'
const ROUTE_DESCRIPTION = 'Um dia inteiro de carro explorando tudo que a Disney oferece de graça: Disney Springs com lojas, restaurantes e entretenimento; os hotéis mais temáticos do mundo onde qualquer pessoa pode entrar e se sentir dentro do universo Disney; o BoardWalk com vista para o EPCOT; resorts com savana africana, arquitetura de hotel de luxo e a magia do monorail passando pelo lobby. Mais de 10 paradas gratuitas que te darão a sensação de estar no mundo Disney sem gastar nada com ingressos.'
...
// ─── Landmarks — circuito completo pelos hotéis e áreas públicas da Disney ───
// Um dia inteiro de carro percorrendo toda a propriedade Disney.
// Todos são gratuitos para visitar (hotéis = lobbies públicos, Springs = entrada livre).

const ROUTE_LANDMARKS: RouteLandmark[] = [
  // ── Disney Springs: ponto de partida ──────────────────────────────
  { name: 'Disney Springs',                             lat: 28.3694, lng: -81.5157 },
  { name: "Disney's Saratoga Springs Resort",           lat: 28.3745, lng: -81.5218 },
  { name: "Disney's Old Key West Resort",               lat: 28.3721, lng: -81.5419 },

  // ── BoardWalk & EPCOT area ─────────────────────────────────────────
  { name: "Disney's BoardWalk Inn",                     lat: 28.3716, lng: -81.5513 },
  { name: "Disney's Yacht & Beach Club Resorts",        lat: 28.3683, lng: -81.5547 },

  // ── Pop Century / Animation Resort (ícones gigantes) ─────────────
  { name: "Disney's Art of Animation Resort",           lat: 28.3551, lng: -81.5359 },
  { name: "Disney's Pop Century Resort",                lat: 28.3572, lng: -81.5330 },
  { name: "Disney's Caribbean Beach Resort",            lat: 28.3587, lng: -81.5428 },

  // ── Animal Kingdom Lodge (savana gratuita no lobby) ───────────────
  { name: "Disney's Animal Kingdom Lodge",              lat: 28.3535, lng: -81.5988 },

  // ── Magic Kingdom corridor ────────────────────────────────────────
  { name: "Disney's Wilderness Lodge",                  lat: 28.4170, lng: -81.5718 },
  { name: "Disney's Fort Wilderness Resort",            lat: 28.4065, lng: -81.5587 },
  { name: "Disney's Polynesian Village Resort",         lat: 28.4087, lng: -81.5836 },
  { name: "Disney's Grand Floridian Resort & Spa",      lat: 28.4103, lng: -81.5866 },
  { name: "Disney's Contemporary Resort (monorail)",    lat: 28.4149, lng: -81.5791 },
]
  ...
  const { data, error } = await db
    ...
    .insert({
      ...
      metadata: {
        ...
        distance,
        duration,
        script:        'create-orlando-disney-free',
