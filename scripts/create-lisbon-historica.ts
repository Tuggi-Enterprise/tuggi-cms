/**
 * create-lisbon-historica.ts
 *
 * Cria a rota "Descubra Lisboa Histórica" no banco de dados.
 *
 * O script:
 *   1. Define 6 landmarks do corredor histórico de Lisboa
 ...
 *
 * Esta é a Fase 1 de aprendizado para depois construir a UI da Fase 2.
 *
 * Uso:
 *   npx tsx scripts/create-lisbon-historica.ts
 *   npx tsx scripts/create-lisbon-historica.ts --force   # recria se já existir
 */
...
const ROUTE_NAME        = 'Descubra Lisboa Histórica'
const ROUTE_DESCRIPTION = 'Rota turística pelo coração histórico de Lisboa: do Marquês de Pombal ao imponente Castelo de São Jorge, passando pelos cartões-postais mais emblemáticos da cidade — Rossio, Praça do Comércio e a Sé Catedral.'
...
// ─── Landmarks do corredor histórico de Lisboa ────────────────────────────────

const ROUTE_LANDMARKS: RouteLandmark[] = [
  { name: 'Marquês de Pombal',            lat: 38.7259, lng: -9.1497 },
  { name: 'Praça dos Restauradores',      lat: 38.7168, lng: -9.1426 },
  { name: 'Rossio (Praça Dom Pedro IV)',  lat: 38.7156, lng: -9.1393 },
  { name: 'Praça do Comércio',           lat: 38.7077, lng: -9.1366 },
  { name: 'Sé de Lisboa',               lat: 38.7097, lng: -9.1318 },
  { name: 'Castelo de São Jorge',        lat: 38.7139, lng: -9.1337 },
]
  ...
  const { data, error } = await db
    ...
    .insert({
      ...
      metadata: {
        ...
        distance,
        duration,
        script:        'create-lisbon-historica',
