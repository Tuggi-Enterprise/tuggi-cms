/**
 * create-acores-nordeste.ts
 *
 * Cria a rota "Nordeste — O Fim do Mundo Verde dos Açores (Faróis e Penhascos)" no banco de dados.
 *
 * O script:
 *   1. Define 8 landmarks da rota do Nordeste de São Miguel
 ...
 *
 * DEMO especial Bensaude Hotels — rota pelo lado mais remoto e selvagem de São Miguel
 *
 * Uso:
 *   npx tsx scripts/create-acores-nordeste.ts
 *   npx tsx scripts/create-acores-nordeste.ts --force   # recria se já existir
 */
...
const ROUTE_NAME        = 'Nordeste — O Fim do Mundo Verde dos Açores (Faróis e Penhascos)'
const ROUTE_DESCRIPTION = 'O Nordeste é o lado mais remoto e selvagem de São Miguel: falésias de 400m cobertas de hortênsias azuis, o farol mais antigo dos Açores (1876), cascatas de água ferruginosa cor de laranja, parques naturais com fetos gigantes e o melhor miradouro para o nascer do sol de toda a ilha. Uma rota de descoberta pelo "fim do mundo verde" que começa na Plantação de Chá Gorreana — a única do continente europeu — e vai subindo a costa até aos penhascos de Ponta da Madrugada. Para hóspedes que querem ver o lado autêntico e não-turístico de São Miguel — sem multidões, com natureza bruta e a sensação de estar no limite do Atlântico.'
...
// ─── Landmarks da rota Nordeste ───────────────────────────────────────────────

const ROUTE_LANDMARKS: RouteLandmark[] = [
  { name: 'Plantação de Chá Gorreana — A Única da Europa (desde 1883)',       lat: 37.8278, lng: -25.5380 },
  { name: 'Salto da Farinha — Cascata de Água Ferruginosa Cor de Laranja',    lat: 37.8330, lng: -25.3100 },
  { name: 'Ribeira dos Caldeirões — Parque Natural com Cascatas e Fetos Gigantes', lat: 37.8200, lng: -25.1530 },
  { name: 'Ponta da Madrugada — Melhor Miradouro de Nascer do Sol dos Açores', lat: 37.8440, lng: -25.1440 },
  { name: 'Ponta do Sossego — Falésia e Vista para o Atlântico Norte',        lat: 37.8260, lng: -25.1480 },
  { name: 'Nordeste Village — Aldeia com Casas Cobertas de Hortênsias',       lat: 37.8265, lng: -25.1567 },
  { name: 'Farol do Arnel — O Mais Antigo dos Açores (1876), Acesso a Pé',   lat: 37.7929, lng: -25.0963 },
  { name: 'Poço Azul — Lago Misterioso Azul no Interior da Mata (Trilho)',    lat: 37.8200, lng: -25.2200 },
]
...
async function insertRoute(
  ...
  const { data, error } = await db
    ...
    .insert({
      ...
      scenic_profile:    ['panoramic', 'nature', 'historical'],
