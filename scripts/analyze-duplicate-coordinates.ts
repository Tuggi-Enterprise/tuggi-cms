...
// Carregar variáveis de ambiente ANTES de importar o cliente Supabase
config()

// Importar cliente Supabase após carregar as variáveis
...
class DuplicateCoordinateAnalyzer {
  ...
  async analyzeDuplicates(): Promise<{
    ...
    // 1. Buscar POIs com múltiplas coordenadas
    ...
    console.log(`📊 Encontradas ${duplicates?.length || 0} coordenadas no total`)

    // 2. Agrupar por attraction_id e identificar duplicatas
    const groupedByAttraction = this.groupByAttraction(duplicates || [])
