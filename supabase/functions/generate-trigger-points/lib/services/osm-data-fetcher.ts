...
import { POIData, BoundaryData, StreetData } from '../types/interfaces.ts';
import { 
...
export class OSMDataFetcher {
  ...
  private extractBuildingHeight(tags: any): number {
    if (!tags) return 4; // Default: casa térrea
    ...
    // 🏙️ ESTRATÉGIA SÊNIOR: No Centro de SP, o erro deve ser para CIMA (bloquear visão)
    const estimates: Record<string, number> = {
      'apartments': 35,  // ~12 andares
      'residential': 15, // ~5 andares
      'commercial': 25,
    ...
    return estimates[buildingType] || 15; 
  }
