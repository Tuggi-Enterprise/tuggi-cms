...
import { POIData, BoundaryData, GeographicContext, DirectionalAnalysis } from '../types/interfaces.ts';
import { calculateBearing, calculateDistance, extractBuildingHeight, findClosestPointOnBoundary } from '../utils/calculations.ts';
...
`;
  ...
  private async findBoundaryPointInDirection(
    ...
    for (const point of boundary.coordinates) {
      ...
      // Simple logic: just use center for now as bearing calculation above is circular
    }
    
    // Simplified: return center (bearing calculation above logic needs review if used for filtering)
    // Actually, let's just use the center for direction filtering to simplify
  ...
  private decideAllowTPs(
    ...
    // A. Bloqueio Duro: Sem ruas acessíveis
    if (streets.accessible === 0) return false;

    // B. Bloqueio Duro: Obstrução Massiva (muitos prédios altos) E visibilidade baixa
    if (visibility.hasObstructions && visibility.score < 0.2) return false;

    // C. Permissão: Visibilidade razoável (> 0.4) ou muitos espaços abertos
    if (visibility.score > 0.4 || openSpaces.percentage > 30) return true;

    // D. Permissão: Poucos prédios (densidade < 50) e altura baixa (< 15m)
    if (buildings.density < 50 && buildings.avgHeight < 15) return true;

    // E. Default: Permitir se houver pelo menos uma rua com algum espaço aberto
    if (streets.withOpenSpaces > 0) return true;
