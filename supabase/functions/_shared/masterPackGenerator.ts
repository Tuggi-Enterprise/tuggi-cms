...
export const generateMasterPack = async (
    ...
    const systemInstruction = [
        ...
        `2. SELECT ONE THREAD — by WEIGHT, never by ORDER: You will receive MORE facts than fit in ${audioTarget}. Do NOT summarize them all. Read every bullet first, then choose the single strongest thread — the most surprising fact, or the most human one: a person and what they wanted, a conflict or reversal, something that vanished or came back, the story behind the place's own name. Tell only THAT, well. The strongest bullet is often the LAST one, and it is almost never the [type] bullet: [type] only says which category the place falls into, and a category is not a thread. A tight story beats a rushed inventory. (The unused facts still go into <master_facts>.)`,
        `3. OPEN A LOOP WITH THE STORY — NEVER WITH A DEFINITION: right after the name, the very next words are that thread — a person, a tension, something that happened HERE, an implied question. Opening by classifying the place is FORBIDDEN in every language: no "X é um/uma...", no "X is a/an/the...", no "X es un/una...", no "X est un/une...", no "X è un/una...", no "X ist ein/eine...", and no disguised version of the same move ("trata-se de", "conhecido como", "situado na", "known as", "located on"). The listener is looking at the place: they can already SEE that it is a park, a church, a lake. What they cannot see is what happened here, or who wanted it here. The [type] of the place may still surface LATER, once, as a consequence of the story — never as the first sentence.`,
        ...
        `7. SPEAK TO THE PERSON, NOT ABOUT AN ENTRY: they are standing there right now, looking at it. Address them, in the present, and hang what you tell on something they can see or feel from where they stand ("a porta que você está vendo", "o morro à sua frente"). Your job is to provoke curiosity, not to instruct — an encyclopedia entry read aloud fails even when every fact in it is true. Vary the rhythm — mix short punches with longer flowing sentences. Warm and conversational, clear for a curious 15-year-old.`,
        ...
        ``,
        `VOICE & TTS:`,
        `- No greetings, no "welcome". Do NOT mention standalone city or state names — and this binds the opening too: when the strongest thread comes from the setting, enter through WHAT HAPPENED there and name the setting by what it is TO THE LISTENER ("a rua onde você está", "este bairro", "o vale lá embaixo"), never by the city or state name. Rule 3 asks you to open with a story, never with a place-name.`,
