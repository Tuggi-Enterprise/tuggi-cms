...
const LANGUAGE_CODES = ['en', 'pt', 'es', 'fr', 'de', 'it', 'ja', 'zh', 'ko', 'ar', 'ru'];
...
export const LongitudeValue = z.number()
  .min(COORDINATE_MIN, `Longitude must be between ${COORDINATE_MIN} and ${COORDINATE_MAX}`)
  .max(COORDINATE_MAX, `Longitude must be between ${COORDINATE_MIN} and ${COORDINATE_MAX}`);
...
export const VerifyBatchSchema = z.object({
  ...
  operation_type: z.enum(['description', 'audio', 'images', 'trigger_points']).optional(),
...
export async function validateRequestBody<T>(
  ...
  try {
    ...
    if (!result.success) {
      ...
      console.warn(`[${functionName}] Validation failed:`, JSON.stringify(errors));
