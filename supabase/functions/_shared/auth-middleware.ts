...
export async function validateAuthHeader(request: Request): Promise<AuthResult> {
  try {
    ...
    if (error || !user) {
      console.warn(`❌ Token validation failed: ${error?.message || 'Invalid token'}`)
      return {
        valid: false,
 ...
 */
export async function requireAuth(request: Request, corsHeaders?: Record<string, string>): Promise<AuthUser | Response> {
...
export async function logAuthEvent(
  ...
  eventType: 'access_granted' | 'access_denied' | 'invalid_token' | 'missing_token',
  userId?: string,
  email?: string,
