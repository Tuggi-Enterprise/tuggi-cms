...
serve(async (req) => {
  ...
  if (!authResult.valid) {
    console.warn(`[Extract-Wikidata-Images] ❌ Unauthorized: ${authResult.error}`)
    return new Response(
      JSON.stringify({ error: 'Unauthorized', detail: authResult.error }),
      { status: 401, headers: createSecureHeaders(corsHeaders) }
    )
  }
  console.log(`[Extract-Wikidata-Images] ✅ Authorized: ${authResult.email}`)

  // ✅ RATE LIMITING CHECK
  const config = RATE_LIMIT_CONFIG['extract-wikidata']
  const rateLimit = checkRateLimit(req, 'extract-wikidata', config.maxRequests, config.windowSeconds)
  if (!rateLimit.allowed) {
    console.warn(`[Extract-Wikidata-Images] ⚠️ Rate limit exceeded for ${rateLimit.clientId}`)
    return createRateLimitResponse(rateLimit, corsHeaders)
  }
  console.log(`[Extract-Wikidata-Images] ✅ Rate limit OK (${rateLimit.remaining} remaining)`)
