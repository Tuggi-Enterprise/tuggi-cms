...
serve(async (req: Request): Promise<Response> => {
    ...
    try {
        ...
        const { action, target_poi, travel_mode, user_context, text_content, hash } = body;
        ...
        if (!user_context?.previous_poi?.id) {
            console.warn(`[Rule] First POI (No Previous POI) - Skipping contextual generation.`);
        ...
        if (!hash && action !== 'wake_up') {
            throw new Error("Missing mandatory 'hash' in request. The mobile app must provide the cache key.");
        }
        if (!target_poi?.id) {
            throw new Error("Missing mandatory 'target_poi.id' in request.");
        }
        if (!user_context?.location) {
            throw new Error("Missing mandatory 'user_context.location' in request.");
        }

        console.log(`[Request] Action: ${action} | POI: ${target_poi.name} (${target_poi.id}) | Hash: ${hash.substring(0, 8)}...`);
        console.log(`[Context] Speed: ${user_context.speed}km/h | Heading: ${user_context.heading.toFixed(1)}° | Platform: ${user_context.platform || 'unknown'} v${user_context.app_version || '?'}`);

        if (user_context.user_profile) {
            console.log(`[User] ${user_context.user_profile.name} (${user_context.user_profile.tier})`);
        }
        ...
        if (directionBucket === 'behind') {
            console.warn(`[Direction Filter] ⚠️ POI BEHIND detected! ${target_poi.name} (Bearing: ${target_poi.bearing.toFixed(1)}°, Heading: ${user_context.heading.toFixed(1)}°). This indicates a frontend calculation issue.`);
            return new Response(JSON.stringify({
                success: false,
        ...
        // ═══════════════════════════════════════════════════════════════════
        if (user_context.next_poi_candidates && user_context.next_poi_candidates.length > 0) {
            ...
            for (const candidate of user_context.next_poi_candidates) {
                ...
                const dist = calculateDistance(
                    user_context.location.latitude, user_context.location.longitude,
            ...
            if (closestCandidate) {
                const headingToCandidate = calculateBearing(
                    user_context.location.latitude, user_context.location.longitude,
                    closestCandidate.location.latitude, closestCandidate.location.longitude
                );
                ...
                console.log(`[Next POI Logic] Selected '${closestCandidate.name}' from ${user_context.next_poi_candidates.length} candidates. Dist: ${Math.round(minDistance)}m`);
            }
        }
        ...
        if (!textContent) {
            console.log(`[Full Gen] No valid text found for ${target_poi.name} (${target_poi.id}). Calling Gemini...`);
            ...
            if (user_context.previous_poi?.location && target_poi.location) {
                ...
                if (!shouldIncludePrevious) {
                    console.log(`[Distance Filter] Excluding previous_poi (${user_context.previous_poi.name}) - Distance to target: ${Math.round(distanceBetweenPOIs)}m > 800m`);
                } else {
                    console.log(`[Distance Filter] Including previous_poi (${user_context.previous_poi.name}) - Distance to target: ${Math.round(distanceBetweenPOIs)}m ≤ 800m`);
                }
            }
        ...
        if (action === 'generate_audio') {
            ...
            if (!textForTTS) {
                throw new Error("Missing text_content for audio generation. Frontend must provide the text.");
            }

            if (!audioUrl) {
                console.log(`[Audio Gen] Generating TTS for ${target_poi.name}...`);
                if (!GOOGLE_TTS_API_KEY) throw new Error("GOOGLE_TTS_API_KEY is missing");
                const audioBuffer = await generateAudioWithTTS(textForTTS, language, 'male', GOOGLE_TTS_API_KEY);
                ...
                console.log(`[Audio Cache Hit] Returning cached audio for ${target_poi.name}`);
            }
        }
