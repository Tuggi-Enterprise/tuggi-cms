-- Create RPC function to efficiently search POIs with verification data
-- This replaces the complex joins in the API route with a single optimized function
...
BEGIN
  ...
  base_query := '
    WITH poi_data AS (
      ...
      LEFT JOIN core.attraction_trigger_points atp ON a.id = atp.attraction_id
      LEFT JOIN core.attraction_group_members agm ON a.id = agm.attraction_id
      LEFT JOIN core.attraction_groups ag ON agm.group_id = ag.id
      WHERE ' || array_to_string(where_conditions, ' AND ') || '
      GROUP BY a.id, a.name, a.city, a.state, a.country, a.google_place_id, a.google_types, 
               a.category, a.rating, a.image_url, a.approved, a.created_at, a.updated_at, 
               a.user_id, a.business_status, a.formatted_phone_number, ac.latitude, ac.longitude
    ),
    stats AS (
      SELECT 
