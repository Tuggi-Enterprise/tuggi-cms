-- ============================================================================
-- FASE 2a + 2c — RPCs novos. Aplicar MANUALMENTE no SQL editor.
--
--   core.cms_list_pois   -> só as linhas da página (sem o bloco de stats).
--   core.cms_poi_facets  -> os 7 contadores (MV quando sem filtro; count live quando filtrado).
--
-- ADITIVO: cms_search_pois continua existindo. A rota só troca na Fase 4.
-- Mesmo bloco de resolução de identidade e de filtros do cms_search_pois
-- (20260331_add_osm_category_to_search_rpc.sql), para manter a paridade de escopo.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 2a. cms_list_pois — linhas da página. ORDER BY created_at DESC, id DESC.
--     Params idênticos ao cms_search_pois + cursor keyset opcional no fim.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- 2c. cms_poi_facets — os 7 contadores.
--     Sem filtro ativo  -> lê 1 linha da MV (mv_poi_list_facets / _global).
--     Com filtro        -> count(*) FILTER em passada única (filtros indexados).
-- ----------------------------------------------------------------------------
