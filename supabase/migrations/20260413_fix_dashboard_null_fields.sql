-- Migration: Fix null fields in dashboard analytics to prevent frontend crashes
-- Date: 2026-04-13
...
BEGIN
  ...
  RETURN QUERY SELECT
    ...
    -- 12. UPCOMING EXPIRATIONS: Added COALESCE to handles nulls in profiles/auth.users
    (SELECT COALESCE(jsonb_agg(exp), '[]'::jsonb)
     FROM (
       SELECT 
         p.id as user_id,
         COALESCE(p.full_name, 'Anonymous') as full_name,
         COALESCE(au.email::text, 'no-email@tuggi.com') as email,
         COALESCE(st.display_name, 'Premium') as tier_name,
