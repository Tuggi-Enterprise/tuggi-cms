...
BEGIN
  ...
  RAISE NOTICE 'RPC called with country_filter: %', country_filter;
  
  -- Build WHERE clause
  IF country_filter IS NOT NULL AND country_filter != '' THEN
    where_clause := ' WHERE a.country = ' || quote_literal(country_filter);
  ...
  EXECUTE format('
    ...
    LIMIT %s
    OFFSET %s
  ', where_clause, limit_count, offset_count);
END;
$$;
