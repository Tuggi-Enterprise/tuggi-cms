-- Fix POI Statistics Calculation - Execute this in Supabase SQL Editor
-- This will fix the cms_search_pois RPC to calculate statistics correctly
...
-- Now let's fix the RPC function
...
-- Create internal implementation to avoid overload ambiguity
...
CREATE OR REPLACE FUNCTION core.cms_search_pois_internal(
  ...
  owner_id TEXT DEFAULT NULL -- Optional: restrict results to POIs created_by this cms_user id
)
RETURNS TABLE (
  -- Basic POI data
  id TEXT,
...
BEGIN
  -- Enforce owner-limiting: if caller is not an admin, force owner_id to the caller's cms_users.id (resolve via email claim)
  ...
  IF NOT is_admin THEN
    -- For non-admins, ignore any provided owner_id and restrict to caller
  ...
  -- Owner filter (optional) - restrict to POIs created by owner_id when provided
...
-- Grant execute permission on internal implementation
...
-- Compatibility: create a 14-parameter wrapper so older callers (or PostgREST caches) that call the
-- original signature continue to work. This wrapper simply forwards to the new function with
-- owner_id := NULL.
DROP FUNCTION IF EXISTS core.cms_search_pois(
  TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, INTEGER, INTEGER, BOOLEAN
);
...
STABLE
AS $$
  -- Forward to internal implementation to avoid overload ambiguity
  SELECT * FROM core.cms_search_pois_internal(
    search_term => search_term,
    status_filter => status_filter,
    country_filter => country_filter,
    state_filter => state_filter,
    city_filter => city_filter,
    google_types_filter => google_types_filter,
    category_filter => category_filter,
    content_status_filter => content_status_filter,
    group_status_filter => group_status_filter,
    score_filter => score_filter,
    trigger_points_filter => trigger_points_filter,
...
-- Ensure authenticated role can execute the compatibility wrapper as well
...
-- Test the compatibility wrapper (14-arg call)
