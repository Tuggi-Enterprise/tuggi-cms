...
BEGIN
  ...
  IF search_term IS NOT NULL AND search_term != '' THEN
    ...
    RAISE NOTICE 'Search term filter applied: %', search_term;
  END IF;

  -- IMPORTANT: Only apply status filter if explicitly requested
  IF status_filter IS NOT NULL AND status_filter != 'all' THEN
    ...
    END IF;
  ELSE
    RAISE NOTICE 'No status filter applied (showing all statuses)';
  END IF;
  ...
  IF state_filter IS NOT NULL AND state_filter != '' THEN
    ...
    RAISE NOTICE 'State filter applied: %', state_filter;
  END IF;
  ...
  IF city_filter IS NOT NULL AND city_filter != '' THEN
    ...
    RAISE NOTICE 'City filter applied: %', city_filter;
  END IF;
  ...
  RAISE NOTICE 'Final filter clause: %', filter_clause;
