-- RPC corrigido para lidar com strings que contêm espaços
...
BEGIN
  ...
  IF search_term IS NOT NULL AND search_term != '' THEN
    ...
    where_conditions := array_append(where_conditions, format('(a.name ILIKE %L OR a.city ILIKE %L OR a.country ILIKE %L)', search_pattern, search_pattern, search_pattern));
  END IF;
  ...
  -- Country filter - usando quote_literal para lidar com espaços
  IF country_filter IS NOT NULL AND country_filter != '' THEN
    where_conditions := array_append(where_conditions, 'a.country = ' || quote_literal(country_filter));
  END IF;

  -- State filter - usando quote_literal para lidar com espaços
  IF state_filter IS NOT NULL AND state_filter != '' THEN
    where_conditions := array_append(where_conditions, 'a.state = ' || quote_literal(state_filter));
  END IF;

  -- City filter - usando quote_literal para lidar com espaços
  IF city_filter IS NOT NULL AND city_filter != '' THEN
    where_conditions := array_append(where_conditions, 'a.city = ' || quote_literal(city_filter));
  END IF;
