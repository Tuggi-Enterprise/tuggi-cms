-- Simplify get_pois_paginated function using KISS principle
-- Remove complex filtering logic and make it straightforward
...
RETURNS TABLE (
  ...
  -- Coordinates from coordinates table
...
DECLARE
  total_rows BIGINT;
BEGIN
  -- Simple count - no complex filtering
  SELECT COUNT(*) INTO total_rows FROM homolog.pois;

  -- Simple query - return all POIs with basic filtering only
  RETURN QUERY
  SELECT
    p.uuid_id,
    p.name,
  ...
  FROM homolog.pois p
  LEFT JOIN homolog.coordinates c ON p.uuid_id = c.poi_uuid_id
