-- Fix the cms_search_pois function with a simpler, more robust approach
...
DECLARE
  ...
  approved_records BIGINT := 0;
  pending_records BIGINT := 0;
  with_description_records BIGINT := 0;
  with_audio_records BIGINT := 0;
  with_trigger_points_records BIGINT := 0;
  complete_records BIGINT := 0;
BEGIN
  ...
  -- Build the main query with a simpler approach
  base_query := '
    WITH filtered_attractions AS (
      SELECT a.*, ac.latitude, ac.longitude
    ...
    poi_data AS (
      SELECT 
        ...
        (
          ...
          WHERE ad_orig.attraction_id = fa.id 
      ...
      LEFT JOIN core.attraction_descriptions ad ON fa.id = ad.attraction_id
      LEFT JOIN core.attraction_trigger_points atp ON fa.id = atp.attraction_id
      LEFT JOIN core.attraction_group_members agm ON fa.id = agm.attraction_id
      LEFT JOIN core.attraction_groups ag ON agm.group_id = ag.id
      GROUP BY fa.id, fa.name, fa.city, fa.state, fa.country, fa.google_place_id, fa.google_types, 
               fa.category, fa.rating, fa.image_url, fa.approved, fa.created_at, fa.updated_at, 
               fa.user_id, fa.business_status, fa.formatted_phone_number, fa.latitude, fa.longitude
    ),
    stats AS (
      SELECT 
        COUNT(*) as total_count,
        COUNT(*) FILTER (WHERE fa.approved = true) as approved_count,
        COUNT(*) FILTER (WHERE fa.approved = false) as pending_count,
        COUNT(*) FILTER (WHERE EXISTS (
          SELECT 1 FROM core.attraction_descriptions ad 
          WHERE ad.attraction_id = fa.id AND ad.description IS NOT NULL AND ad.description != ''''
        )) as with_description_count,
        COUNT(*) FILTER (WHERE EXISTS (
          SELECT 1 FROM core.attraction_descriptions ad 
          WHERE ad.attraction_id = fa.id AND ad.audio_url IS NOT NULL AND ad.audio_url != ''''
        )) as with_audio_count,
        COUNT(*) FILTER (WHERE EXISTS (
          SELECT 1 FROM core.attraction_trigger_points atp 
          WHERE atp.attraction_id = fa.id
        )) as with_trigger_points_count,
        COUNT(*) FILTER (WHERE EXISTS (
          SELECT 1 FROM core.attraction_descriptions ad 
          WHERE ad.attraction_id = fa.id AND ad.description IS NOT NULL AND ad.description != '''' 
            AND ad.audio_url IS NOT NULL AND ad.audio_url != ''''
        )) as complete_count
      FROM filtered_attractions fa
    )
    SELECT 
      ...
      s.approved_count,
      s.pending_count,
      s.with_description_count,
      s.with_audio_count,
      s.with_trigger_points_count,
      s.complete_count
    FROM poi_data pd
    CROSS JOIN stats s
    ' || order_clause;
