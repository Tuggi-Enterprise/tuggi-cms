...
async function reprocessUnprocessedItems() {
  ...
  try {
    ...
    // Processar os dados para obter o score mais recente de cada item (se existir)
    const processedItems = unprocessedItems.map(item => {
    ...
    for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
      ...
      for (let i = 0; i < currentBatch.length; i++) {
        ...
        try {
          ...
          console.log(`   📊 Score atual: ${oldScore ? oldScore + '%' : 'N/A'} | Status: ${oldStatus || 'null'}`);
          ...
          if (error) {
            ...
            const scoreDiff = oldScore !== null ? newScore - oldScore : null;
            const scoreChangeEmoji = scoreDiff === null ? '🆕' : scoreDiff > 0 ? '📈' : scoreDiff < 0 ? '📉' : '➡️';
            
            console.log(`   ✅ Sucesso!`);
            if (oldScore !== null) {
              console.log(`   📊 Score: ${oldScore}% → ${newScore}% ${scoreChangeEmoji} (${scoreDiff > 0 ? '+' : ''}${scoreDiff}%)`);
            } else {
              console.log(`   📊 Score: N/A → ${newScore}% ${scoreChangeEmoji} (primeira verificação)`);
            }
            console.log(`   🏷️ Status: ${oldStatus || 'null'} → ${data.verification_status || 'unknown'}`);
    ...
    if (scoreChanges.length > 0) {
      ...
      const improved = scoreChanges.filter(change => change.scoreDiff !== null && change.scoreDiff > 0);
      const worsened = scoreChanges.filter(change => change.scoreDiff !== null && change.scoreDiff < 0);
      const unchanged = scoreChanges.filter(change => change.scoreDiff === 0);
      const newItems = scoreChanges.filter(change => change.scoreDiff === null);
      ...
      console.log(`🆕 Primeira verificação: ${newItems.length} itens`);
