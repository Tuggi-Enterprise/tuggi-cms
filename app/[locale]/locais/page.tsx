...
import {
  ...
  Clock, CheckCircle2, Hourglass, ChevronLeft, ChevronRight,
} from 'lucide-react'
      ...
      <div className="flex gap-8">
        ...
        <div className="w-[82%] min-w-0">
          <StatCardRow columns={4} className="mb-6">
            <StatCard label={t('stats.total')} value={facets?.total ?? 0} icon={Store} color="#00A8E8" isLoading={!facets} />
            <StatCard label={t('stats.with_hours')} value={facets?.withHours ?? 0} icon={Clock} color="#8B5CF6" isLoading={!facets} />
            <StatCard label={t('stats.approved')} value={facets?.approved ?? 0} icon={CheckCircle2} color="#10B981" isLoading={!facets} />
            <StatCard label={t('stats.pending')} value={facets?.pending ?? 0} icon={Hourglass} color="#FF6F00" isLoading={!facets} />
          </StatCardRow>
          ...
          <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl rounded-3xl border border-gray-200 dark:border-gray-800 shadow-2xl shadow-black/5 mb-6">
            <div className="p-4 flex items-center justify-between">
              <div className="flex items-center gap-4 pl-2">
                <div className="p-2.5 bg-tuggi-blue/10 rounded-2xl"><Store className="h-5 w-5 text-tuggi-blue" /></div>
          ...
          {isLoading ? (
            ...
            <div className="space-y-3">
              {items.map((pl) => (
                ...
                >
                  <div className={cn('absolute top-0 left-0 w-1 h-full', pl.approved ? 'bg-gradient-to-b from-green-400 to-emerald-500' : 'bg-gradient-to-b from-orange-400 to-amber-500')} />
                  <div className="flex items-center justify-between gap-4 pl-2">
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-3 mt-1 text-xs text-gray-500">
                        {pl.place_type && (
                          <>
                            <span className="capitalize">{pl.place_type}{pl.price_range ? <span className="ml-1.5 text-tuggi-blue font-semibold">{'$'.repeat(pl.price_range)}</span> : null}</span>
                            <span className="text-gray-300 dark:text-gray-700">•</span>
                          </>
                        )}
                        <span className="truncate">{[pl.city, pl.state, pl.country].filter(Boolean).join(', ')}</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-3 flex-shrink-0">
                      {pl.has_hours && <span className="hidden sm:inline text-[11px] text-gray-400">{t('badges.hours')}</span>}
                      <span className="hidden md:inline text-[11px] text-gray-400">{t('content_summary', { desc: pl.description_count, tp: pl.trigger_point_count })}</span>
