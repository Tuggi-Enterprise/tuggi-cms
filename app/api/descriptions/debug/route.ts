...
export async function POST(request: NextRequest) {
  try {
    console.log('🚀 DEBUG: Starting description generation...')
    ...
    console.log('✅ DEBUG: Required parameters check:', { name: !!name, city: !!city, country: !!country })
    if (!name || !city || !country) {
      console.error('❌ DEBUG: Missing required parameters:', { name: !!name, city: !!city, country: !!country })
    ...
    console.log('🔑 DEBUG: API Key check:', apiKey ? 'Configured' : 'NOT CONFIGURED')
    if (!apiKey) {
      console.error('❌ DEBUG: Gemini API key not configured')
      return NextResponse.json(
...
Write only the description in Portuguese, no additional text.`
    ...
    if (!response.ok) {
      ...
      console.error('❌ DEBUG: Gemini API error:', errorData)
      return NextResponse.json(
    ...
    console.log('📄 DEBUG: Gemini API response data:', JSON.stringify(data, null, 2))
    
    const generatedText = data.candidates?.[0]?.content?.parts?.[0]?.text
    
    if (!generatedText) {
      console.error('❌ DEBUG: No text generated by Gemini')
      return NextResponse.json(
    ...
    console.log('✅ DEBUG: Successfully generated description:', generatedText)
    ...
    console.error('❌ DEBUG: Error generating description:', error)
